...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
   ...
   if (DEBUG_FLUSH) {
      when (flush_val) {
         printf(p"Flush because of exception[$exception_thrown] inst flush [$flush_commit]\n")
      }
   }
   ...
   println(s"len(r_xcpt_badvaddr) = ${r_xcpt_badvaddr.getWidth}," +
     ...
     s" len(JAL target) = ${ComputeJALTarget(io.enq_uops(0).pc, io.enq_uops(0).debug_inst, xLen).getWidth}")
